...
const Period = () => {
	const breadcrumbItems = [
		...
		{ label: "Period", path: "/NWF/category/period", active: true },
	];
	...
	function getPeriod(page, limit, form = {}) {
		const payload = {
			limit,
			page,
			...form,
		...
		request
			...
			.catch((e) => setPeriodList([]));
	}

	const search = (form) => {
		setLimit(10);
		setPage(1);
		getPeriod(page, limit, form);
	};
	...
	function onPageChange(page) {
		setPage(page);
		getPeriod(page, limit, searchForm);
	}

	return (
		<div class="container-fluid mt-2">
			<Pagetitle breadcrumbItems={breadcrumbItems} title={"Quản lý kỳ hạn"}></Pagetitle>
			{mode === "TABLE" ? (
				<div class="row">
					<div class="col-12 card-box">
						<fieldset>
							...
							{isCollapsed && (
								<Form>
									<div className="row">
										<div className="col-4">
											<InputComponent title={"Mã kỳ hạn"} name="code" value={code} onChange={(val) => setCode(val)}></InputComponent>
										</div>

										<div className="col-4">
											<InputComponent title={"Tên kỳ hạn"} name="name" value={name} onChange={(val) => setName(val)}></InputComponent>
										</div>
										<div className="col-4">
											<SelectComponent
												notFirstDefault
						...
						<div className="col-12 border-bottom-dotted pb-0 p-0 mb-2 mt-3">
							<span className="font-weight-medium theme-color">Danh sách kỳ hạn</span>
						</div>
							...
							<table className="table table-bordered table-sm table-hover m-w-tabble">
								<thead>
									<tr className="m-header-table">
										<th className="text-center align-middle mw-150">Mã kỳ hạn</th>
										<th className="text-center align-middle mw-200">Tên kỳ hạn</th>
