...
const FeeGroupAction = (props) => {
	...
	function create(form) {
		...
		console.log(json);
		request
			.post(api.CREATE_FEE_GROUP, json)
			.then((res) => {
				setLoading(false);
				if (res.errorCode === "0") {
					passEntry();
	...
	function update(form) {
		...
		request
			.post(api.UPDATE_FEE_GROUP, json)
			.then((res) => {
				setLoading(false);
				if (res.errorCode === "0") {
			...
			<div className="row">
				<div className="col-12 card-box">
					<div class="col-12 border-bottom-dotted p-0 mb-2">
						<span class="font-weight-medium theme-color">{item ? "Thông tin cập nhật nhóm phí" : "Thông tin tạo nhóm phí"}</span>
					</div>
					<Form>
						<div className="row">
							<div className="col-4">
